...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate coverage for world/phasing duplicate-name variants."
    )
    parser.add_argument(
        "--families",
    ...
    parser.add_argument(
        "--out",
        default="/mnt/d/Battlenet/World of Warcraft/_retail_/Interface/AddOns/Repu/data/retail/coverage_world_variants_generated.lua",
        help="Where to write the generated world-variant coverage module.",
    )
...
def collect_rows(manifest: dict) -> tuple[list[dict], list[dict]]:
    ...
    for family in manifest["families"]:
        if family["familyClass"] != "world_or_phasing_variant":
            continue
...
def render_module(zone_rows: list[dict], subzone_rows: list[dict]) -> str:
    ...
    lines.append("")
    lines.append("-- Generated from grouped world/phasing duplicate-name variants.")
    lines.append("-- These entries extend location coverage for outdoor/phased variants without inventing faction mappings.")
    ...
    for row in zone_rows:
        ...
        lines.append(
            '                tags = { "zone", "coverage-only", "generated-variant", "world-phasing-variant", "'
            + group_name
            + '" },'
        )
        lines.append("            },")
    ...
    for row in subzone_rows:
        ...
        lines.append(f"                mapIDs = {{ {row['mapID']} }},")
        lines.append(
            '                tags = { "subzone", "coverage-only", "generated-variant", "world-phasing-variant", "'
            + group_name
            + '" },'
        )
        lines.append("            },")
...
def main() -> int:
    ...
    print(f"Generated world/phasing variant zones: {len(zone_rows)}")
    print(f"Generated world/phasing variant subZones: {len(subzone_rows)}")
